    ...
    <>
      ...
      <SectionMain>
        ...
        <CardBox>
          ...
          <>
            <p className={'block font-bold mb-2'}>Bookings Customer</p>
            <CardBox
              className='mb-6 border border-gray-300 rounded overflow-hidden'
              hasTable
            >
              <div className='overflow-x-auto'>
                <table>
                  ...
                  <tbody>
                    {users.bookings_customer &&
                      Array.isArray(users.bookings_customer) &&
                      users.bookings_customer.map((item: any) => (
                        <tr
                          key={item.id}
                          onClick={() =>
                            router.push(
              ...
              {!users?.bookings_customer?.length && (
                <div className={'text-center py-4'}>No data</div>
              )}
            </CardBox>
          </>
          ...
          <>
            <p className={'block font-bold mb-2'}>Buses Driver</p>
            <CardBox
              className='mb-6 border border-gray-300 rounded overflow-hidden'
              hasTable
            >
              <div className='overflow-x-auto'>
                <table>
                  ...
                  <tbody>
                    {users.buses_driver &&
                      Array.isArray(users.buses_driver) &&
                      users.buses_driver.map((item: any) => (
                        <tr
                          key={item.id}
                          onClick={() =>
                            router.push(`/buses/buses-view/?id=${item.id}`)
                          }
                        >
                          <td data-label='bus_number'>{item.bus_number}</td>

                          <td data-label='operator_name'>
                            {item.operator_name}
                          </td>
                        </tr>
                      ))}
