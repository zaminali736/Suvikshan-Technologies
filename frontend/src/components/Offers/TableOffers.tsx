...
import {
  ...
  setRefetch,
  deleteItemsByIds,
} from '../../stores/offers/offersSlice';
import { useAppDispatch, useAppSelector } from '../../stores/hooks';
import { useRouter } from 'next/router';
import { Field, Form, Formik } from 'formik';
import { DataGrid, GridColDef } from '@mui/x-data-grid';
import { loadColumns } from './configureOffersCols';
...
const TableSampleOffers = ({
  ...
  useEffect(() => {
    if (offersNotify.showNotification) {
      notify(offersNotify.typeNotification, offersNotify.textNotification);
    }
  }, [offersNotify.showNotification]);
  ...
  const handleViewAction = (id: string) => {
    router.push(`/offers/offers-view/?id=${id}`);
  };

  const handleCreateEventAction = ({ start, end }: SlotInfo) => {
    router.push(
      `/offers/offers-new?dateRangeStart=${start.toISOString()}&dateRangeEnd=${end.toISOString()}`,
    );
  };
    ...
    <>
      ...
      {!showGrid && (
        <BigCalendar
          events={offers}
          showField={'title'}
          start-data-key={'valid_from'}
          end-data-key={'valid_until'}
          handleViewAction={handleViewAction}
          handleEditAction={handleEditAction}
          handleDeleteAction={handleDeleteModalAction}
          pathEdit={`/offers/offers-edit/?id=`}
          pathView={`/offers/offers-view/?id=`}
...
export default TableSampleOffers;
