...
const ListBuses = ({
  ...
  const hasUpdatePermission = hasPermission(currentUser, 'UPDATE_BUSES');
              ...
              >
                ...
                >
                  <div className={'flex-1 px-3'}>
                    ...
                    <p className={'line-clamp-2'}>{item.bus_number}</p>
                  </div>

                  <div className={'flex-1 px-3'}>
                    <p className={'text-xs   text-gray-500 '}>OperatorName</p>
                    <p className={'line-clamp-2'}>{item.operator_name}</p>
                  </div>

                  <div className={'flex-1 px-3'}>
                    <p className={'text-xs   text-gray-500 '}>Driver</p>
                    <p className={'line-clamp-2'}>
                      {dataFormatter.usersOneListFormatter(item.driver)}
                    </p>
                  </div>
                </div>
                <ListActionsPopover
                  ...
                  onEdit={onEdit}
                  itemId={item.id}
                  pathEdit={`/buses/buses-edit/?id=${item.id}`}
                  pathView={`/buses/buses-view/?id=${item.id}`}
