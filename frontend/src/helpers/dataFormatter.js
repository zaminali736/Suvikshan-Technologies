...
export default {
  ...
  busesManyListFormatter(val) {
    if (!val || !val.length) return [];
    return val.map((item) => item.bus_number);
  },
  busesOneListFormatter(val) {
    if (!val) return '';
    return val.bus_number;
  },
  busesManyListFormatterEdit(val) {
    if (!val || !val.length) return [];
    return val.map((item) => {
      return { id: item.id, label: item.bus_number };
    });
  },
  busesOneListFormatterEdit(val) {
    if (!val) return '';
    return { label: val.bus_number, id: val.id };
  },
